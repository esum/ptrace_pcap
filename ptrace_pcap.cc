...
inline void write_ip4_header(int fd, struct in_addr const *src_addr, struct in_addr const *dst_addr, unsigned short total_length)
{
...
inline void write_ip6_header(int fd, struct in6_addr const *src_addr, struct in6_addr const *dst_addr, unsigned short payload_length)
{
			...
			switch (regs.orig_rax) {
				case __NR_read: {
					...
					if (ip4) {
						...
						if (cur_local_addr.contains(regs.rdi) && cur_remote_addr[regs.rdi].first.size() == 16) {
							for (int i = 0; i < 16; i++)
								dst_addr.s6_addr[i] = cur_local_addr[regs.rdi].first[i];
						}
						else
				...
				case __NR_write: {
					...
					if (ip4) {
						...
						if (cur_local_addr.contains(regs.rdi) && cur_remote_addr[regs.rdi].first.size() == 16) {
							for (int i = 0; i < 16; i++)
								src_addr.s6_addr[i] = cur_local_addr[regs.rdi].first[i];
						}
						else
							memset(&src_addr, 0, sizeof(struct in6_addr));
						if (cur_remote_addr.contains(regs.rdi) && cur_remote_addr[regs.rdi].first.size() == 16) {
					...
					if (last_syscall.contains(regs.rdi) && last_syscall[regs.rdi] != regs.orig_rax)
						write_tcp_header(dump, cur_local_addr.contains(regs.rdi) ? cur_local_addr[regs.rdi].second : default_src_port, cur_remote_addr.contains(regs.rdi) ? cur_remote_addr[regs.rdi].second : default_dst_port, seq_out[regs.rdi], seq_in[regs.rdi], 16);
					else
						write_tcp_header(dump, cur_local_addr.contains(regs.rdi) ? cur_local_addr[regs.rdi].second : default_src_port, cur_remote_addr.contains(regs.rdi) ? cur_remote_addr[regs.rdi].second : default_dst_port, seq_out[regs.rdi], 0, 0);
					write(dump, buffer, regs.rax);
